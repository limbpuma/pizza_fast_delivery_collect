...
interface EnhancedPizzaModalProps {
  isOpen: boolean;
  onClose: () => void;
  pizza: any;
...
function EnhancedPizzaModal({ isOpen, onClose, pizza, saucesOptions }: EnhancedPizzaModalProps) {
  ...
  const calculateTotalPrice = (): number => {
    ...
    const itemTotal = (basePrice + saucePrice) * quantity;
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const itemName = `${name} (${sizeName}, ${sauceName})`;
      
      const newItem = {
        pizzaId: id,
        name: itemName,
        quantity: quantity,
        unitPrice: selectedSizeData.price + selectedSauceData.price,
...
export default EnhancedPizzaModal;
