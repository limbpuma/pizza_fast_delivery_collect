...
try {
  ...
  // El archivo menu-real-campus.json es una muestra con objetos individuales, no un array
  const realMenuContent = fs.readFileSync('./public/data/menu-real-campus.json', 'utf8');
  
  // Extraer todos los objetos del archivo y convertirlos en un array
  const realMenuObjects = realMenuContent
    .split('\n')
    .filter(line => line.trim().startsWith('"artikelNr":'))
    .map(line => {
      // Buscar el objeto completo que contiene esta línea
      const startIndex = realMenuContent.indexOf(line) - 50; // Buscar hacia atrás
      const objStart = realMenuContent.lastIndexOf('{', realMenuContent.indexOf(line));
      const objEnd = realMenuContent.indexOf('}', realMenuContent.indexOf(line)) + 1;
      const objString = realMenuContent.substring(objStart, objEnd);
      try {
  ...
  console.log(`✅ Archivo real procesado: ${realMenuObjects.length} items encontrados`);
